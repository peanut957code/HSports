        ...
        <div class="w-full max-w-lg">
            ...
            <div class="bg-[#fff] p-4">
                <div class="flex flex-col">
                    ...
                    <el-input size="large" type="number" min="1" max="99" v-model="venueObj.num" :placeholder="$t('creaSite.completeNumber')" />
                </div>
            </div>
            ...
            <div class="bg-[#fff] px-4 mb-[50px]">
                <div v-for="(schedule, index) in schedules" :key="index" class="flex items-center mb-[10px]">
...
import { add } from '@/api/venue/index';
...
const confirmTime = () => {
    if (currentField.value === 'begin') {
        ...
        schedules[currentIndex.value].begin = `${currentTime.value[0]}:${currentTime.value[1]}`;
    } else if (currentField.value === 'end') {
        schedules[currentIndex.value].end = `${currentTime.value[0]}:${currentTime.value[1]}`;
    }
...
// 添加时间段
const addSchedule = () => {
    schedules.push({ begin: '', end: '', price: '' });
};
